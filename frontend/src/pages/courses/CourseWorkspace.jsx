...
export default function CourseWorkspace({ role, session }) {
  ...
  return (
    <PlatformLayout
      role={accentRole}
      eyebrow={isAdmin ? "Admin Course Workspace" : "Faculty Course"}
      ...
      subtitle={
        isAdmin
          ...
          : "Create assignments, publish course problems, upload material, and review the enrolled roster."
      }
      meta={isAdmin ? "Admin Managed Course" : "Assigned Faculty"}
      actions={
      ...
      sidebarNote={
        isAdmin
          ...
          : "Faculty access is scoped to assigned courses only, so course creation stays with admins while content management stays with faculty."
      }
    >
      ...
      {!status.loading && !status.error && data ? (
        <>
          ...
          <PlatformSection label="Overview" title="Complete course details">
            <div className="history-list">
              <article className="history-card">
                <strong>Course description</strong>
                <p>{data.description || "No description added yet."}</p>
                <p className="question-meta">
                  Branches: {data.branchTargets.join(", ")} | Semesters: {data.semesterTargets.join(", ")} | Sections:{" "}
                  {data.sectionTargets.join(", ")} | Batches: {data.batchTargets.join(", ")}
                </p>
                <p className="question-meta">
          ...
          <PlatformSection label="Published Assignments" title="Current assignment list">
              ...
              <div className="history-list">
                {data.assignments.map((assignment) => (
                  <article className="history-card" key={assignment.id}>
                    ...
                    <p className="question-meta">
                      {assignment.type} | Due {assignment.dueDate ? new Date(assignment.dueDate).toLocaleString() : "No deadline"} | Max score{" "}
                      {assignment.maxScore}
                    </p>
                  </article>
                ))}
          ...
          <PlatformSection label="Study Material" title="Uploaded resources">
          ...
          <PlatformSection label="Coding Problems" title="Course question bank">
                ...
                {data.codingProblems.map((problem) => (
                  <article className="history-card" key={problem.id}>
                    ...
                    <p className="question-meta">
                      ...
                      {isAdmin || role === "faculty" ? ` | Hidden cases: ${problem.hiddenTestCases?.length || 0}` : ""}
                    </p>
